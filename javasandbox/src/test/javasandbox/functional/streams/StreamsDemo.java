...
public class StreamsDemo {
	...
	private static void declarative() {
		 ...
		 * (b) 0 or more intermediate operations (~ WHERE clause) -- lazy & return
		 * Stream<T>, i.e., transforms a stream into another stream
		 * (c) terminal operation (~ column names) -- eager & return NON-STREAM.
		 * Terminates (closes) a stream
		 * 
		 */
...
//				.forEach(System.out::println);
	...
	private static void print(Stream<String> stream) {
		...
		// error as stream already closed
		// stream.forEach(System.out::println);
	}
