package test.javasandbox.datetime;
...
public class DateTimeDemo {
	private static void testLegacyDateAPI() {
		...
		gameStartTime.setTimeZone(TimeZone.getTimeZone("Asia/Manila"));
		System.out.println("Philippine time -- MONTH/DAY at hr:min:sec (AM/PM) -- "
				+ (gameStartTime.get(Calendar.MONTH) + 1) + "/" + gameStartTime.get(Calendar.DAY_OF_MONTH) + " at "
				+ gameStartTime.get(Calendar.HOUR) + ":" + gameStartTime.get(Calendar.MINUTE) + " ("
				+ ((gameStartTime.get(Calendar.AM_PM) == 0) ? "AM" : "PM") + ")");
	...
	private static void testDateTimeAPI() {
		...
		ZonedDateTime philippinesTime = zonedGameStartTime.withZoneSameInstant(ZoneId.of("Asia/Manila"));
		System.out.println("philippinesTime: " + philippinesTime);
		...
		// System.nanoTime() more recommended
